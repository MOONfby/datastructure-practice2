...
typedef struct TWSTACK {    //静态分配
     ElemType elem[N];
     int top1,top2;
     } TWSTACK;
void inistack(TWSTACK *S);            //初始化
int push(TWSTACK *S,int i,ElemType e);//增加元素
int pop(TWSTACK *S,int i, ElemType e);//删除元素
int main()
{
int  i,f,e1,e2;
TWSTACK S;
...
void inistack(TWSTACK *S)//该函数实现初始化S，得到2个空栈。根据双向栈的示意图，理解初始化要求。
{
   S->top1=0;
   S->top2=N-1;
}
